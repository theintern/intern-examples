...
define([
	...
	return {
		 ...
		 * @param onload A function to call with the value for name. This tells the loader that the plugin is done loading
		 * 		  the resource. onload.error() can be called, passing an error object to it, if the plugin detects an
		 * 		  error condition that means the resource will fail to load correctly.
		 */
		load: function(name, req, onload) {
			...
			request(req.toUrl(name)).then(function (sourceCode) {
				...
				// …then execute the compiled function. In this case the example code
				// puts things into the global space so it needs to be ran in a script tag.
